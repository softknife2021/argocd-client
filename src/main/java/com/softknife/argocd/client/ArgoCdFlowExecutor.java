...
//import com.softknife.argocd.client.model.post.job.PostBuild;
//import com.softknife.argocd.client.model.task.BuildExecResult;
//import com.softknife.argocd.client.model.task.BuildExecutorTask;
...
// */
//public class ArgoCdFlowExecutor {
//
...
//
//    public ArgoCdFlowExecutor(BuildExecutorTask buildExecutorTask, ArgoCdClient argoCdClient) {
...
//        try {
//            Response triggerJobResp = this.argoCdClient.postBuild(ArgoClientResourceManager.getInstance().getObjectMapper().writeValueAsString(postBuild));
//            if (triggerJobResp == null) {
//                setExecutionResults(buildExecResult, TaskState.ABORTED.getValue(), ArgoCdConstant.ERROR_FAILED_TO_TRIGGER_BUILD, postBuild);
//            } else {
...
//                    if (whatIsBuildState(buildId).equalsIgnoreCase(ArgoCdConstant.BUILD_STATE_QUEUED)) {
//                        setExecutionResults(buildExecResult, TaskState.ABORTED.getValue(), ArgoCdConstant.ERROR_QUEUE_EXCEEDED_TIME, postBuild);
...
//            if (buildState.equalsIgnoreCase(ArgoCdConstant.BUILD_STATE_QUEUED)) {
...
//        return Integer.toString(read(metaData, ArgoCdConstant.JSON_PATH_BUILD_QUEUE_ID));
//    }
//
//    private String whatIsBuildState(String buildQueueId) {
//        String responseBody = this.getBuildMetaData(buildQueueId);
//        return read(responseBody, ArgoCdConstant.JSON_PATH_BUILD_STATE);
//    }
//
//    private String getBuildMetaData(String buildId) {
//        String buildMetaData = null;
//        try {
//            Response response = this.argoCdClient.getBuildById(buildId);
//            if (response != null) {
...
//            if (buildState.equalsIgnoreCase(ArgoCdConstant.BUILD_STATE_RUNNING)) {
...
//                String status = JsonPath.read(buildExecResult.getExecutionMetaData(), ArgoCdConstant.JSON_PATH_BUILD_STATUS);
//                if (status.equalsIgnoreCase(ArgoCdConstant.BUILD_STATUS_FAILURE) || status.equalsIgnoreCase(ArgoCdConstant.BUILD_STATUS_UNKNOWN)) {
//                    setBuildErrors(postBuild.getBuildType().getBuildTypeId(), ArgoCdConstant.ERROR_PRIORITY_BUILD_FAILURE);
...
//                if (entry.getValue().getState().equalsIgnoreCase(ArgoCdConstant.BUILD_STATE_FINISHED)) {
//                    try {
//                        buildStatus = JsonPath.read(entry.getValue().getExecutionMetaData(), ArgoCdConstant.JSON_PATH_BUILD_STATUS);
//                        if (buildStatus == null || !buildStatus.equalsIgnoreCase(ArgoCdConstant.BUILD_STATUS_SUCCESS)) {
